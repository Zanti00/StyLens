...
export const AnalysisHistory: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {[1, 2, 3].map((i) => (
          <div key={i} className="bg-white rounded-2xl h-80 animate-pulse border border-zinc-100" />
        ))}
      </div>
    );
  }
  ...
  if (items.length === 0) {
    return (
      <div className="text-center py-20 bg-white rounded-3xl border border-zinc-100">
        ...
        <h3 className="text-xl font-bold text-zinc-800">No history yet</h3>
        <p className="text-zinc-500 mt-2">Your analyzed outfits will appear here.</p>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {items.map((item) => (
        <div key={item.id} className="group bg-white rounded-3xl overflow-hidden border border-zinc-100 shadow-sm hover:shadow-xl hover:shadow-indigo-50/50 transition-all duration-500 hover:-translate-y-1">
          {/* Image Container */}
          <div className="relative aspect-[4/5] overflow-hidden">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-6">
               <button className="w-full bg-white text-zinc-900 py-3 rounded-xl font-bold flex items-center justify-center gap-2 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
            ...
            {item.rating && (
              <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-md px-3 py-1.5 rounded-full flex items-center gap-1.5 shadow-sm">
                ...
                <span className="text-sm font-bold text-zinc-900">{item.rating}/10</span>
              </div>
            )}
          </div>
          ...
          <div className="p-6 space-y-4">
            ...
            <div className="pt-2 flex items-center justify-between border-t border-zinc-50">
               <p className="text-sm text-zinc-400 italic truncate pr-4">
